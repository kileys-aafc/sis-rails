...
# (the path to perl)
#
#  experimental script to cache files
# 
...
if ($lang =~ /English/) { 
   print <<"EOF";
  <HTML>
  <HEAD><TITLE>Center Map Index by Name</TITLE>
  </HEAD>
  <BODY>
  <CENTER><IMG SRC="/CANSIS/logo.gif">
  <H1>NSDB<BR>Center Map Index by Name</H1></CENTER>
  <HR>
...
#
      if ($found_some =~ /1/)
   {
      ...
      if($name eq "UNIQUE_KEY")
        ...
        print "<TD><A HREF=\"http://hp260/bin/scripts/.esrimap?name=Index&Cmd=Locate";
        print "&LL=$lat_deg-$lat_min-$lat_sec";
        print "N,$long_deg-$long_min-$long_sec";
        print "W&placename=$feature_name2";
